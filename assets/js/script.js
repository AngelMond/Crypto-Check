               //Code for converting the Mexican pesos to US Dollars
        const options = {
                method: 'GET',
                headers: {
...
var exchangeValue = getCurrency()
.then((response) => {
    return response.json();
            })
            .then((data) => {
                console.log('inside the getCurrency()')
                console.log(data.new_amount);

                mexicanPesosToDollars = data.new_amount;
                console.log('Hola: ' + mexicanPesosToDollars)
       ...
       //Select Tag to append option tag's
        ...
        mexicanPesos.html('$');
        // mexicanPesos.append(mexicanPesosToDollars);
...
console.log('Value of the exchange: ' + exchangeValue)
